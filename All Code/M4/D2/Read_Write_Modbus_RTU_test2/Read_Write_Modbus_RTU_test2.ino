...
void readBoard() {
  ...
  delay(10);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, slaveID);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, 0x02);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, 0x00);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, 0x00);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, 0x00);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, 0x08);
  h_byteCRC = highByte(calculateCRC);
  l_byteCRC = lowByte(calculateCRC);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, l_byteCRC);
  calculateCRC = sendByte_CRCUpdate(calculateCRC, h_byteCRC);
...
void loop() {
  relayCTRL(0, relay_on); delay(1500); readBoard(); delay(1500);
  relayCTRL(0, relay_off); delay(1500); readBoard(); delay(1500);
  relayCTRL(1, relay_on); delay(1500); readBoard(); delay(1500);
  relayCTRL(1, relay_off); delay(1500); readBoard(); delay(1500);
}
